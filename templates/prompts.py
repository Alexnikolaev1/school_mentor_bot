...
ENCYCLOPEDIA_MODE_LABELS = {
    "brief": "Краткий справочник",
    "article": "Полная статья",
    "timeline": "Хронология",
    "connections": "Карта связей",
    "deepen": "Углублённый разбор",
}


def encyclopedia_prompt(
    topic: str,
    mode: str,
    grade: int,
    level: str,
    ...
    level_name = LEVELS.get(level, "средний")
    mode_label = ENCYCLOPEDIA_MODE_LABELS.get(mode, "Справочник")
    context = (
        f"\nУже показано ученику (не повторяй дословно, дополни):\n{previous_text[:1500]}\n"
        if previous_text else ""
    )

    mode_instructions = {
        "brief": """Формат «Краткий справочник»:
📌 Определение (1-2 предложения)
🔑 5-7 ключевых фактов (нумерованный список)
💡 Зачем это знать школьнику (1 предложение)
📎 2 темы для углубления""",
        "article": """Формат «Полная статья»:
📌 Введение и определение
📚 Основные разделы (3-5 подзаголовков с содержанием)
⭐ Важные даты, имена, термины
🎯 Вывод: 3 главных тезиса
📎 Что изучить дальше""",
        "timeline": """Формат «Хронология»:
📌 Краткое введение к теме
🕐 Хронологическая лента (от раннего к позднему): дата/период — событие — значение
🔗 2-3 причинно-следственные связи между событиями
💡 Итог: почему эта хронология важна""",
        "connections": """Формат «Карта связей»:
📌 Центральная тема в 1 предложении
🔗 6-8 связанных понятий/тем с кратким пояснением связи
🌐 Межпредметные связи (история, биология, литература и т.д.)
📚 Маршрут изучения: в каком порядке пройти связанные темы""",
        "deepen": """Формат «Углублённый разбор»:
📌 Новые детали, которых не было в базовом материале
🔬 Углубление: причины, механизмы, исключения, спорные моменты
📊 Сравнения, таблицы или противопоставления (текстом)
🎓 Уровень чуть выше школьной программы, но понятно""",
    }

    instructions = mode_instructions.get(mode, mode_instructions["brief"])

    return f"""Ты — школьная энциклопедия и методист.
Подготовь материал по теме «{topic}» для ученика {grade} класса ({level_name} уровень).
Режим: {mode_label}.
{context}
{instructions}

Требования:
- Только проверенные факты, без выдумок
- Язык понятный школьнику, без академической воды
- Используй эмодзи для структуры
- Длина: 250-450 слов (кроме краткого справочника — до 200 слов)
- Если тема неоднозначна — уточни, о каком аспекте пишешь, в первом абзаце

Тема: {topic}"""


def encyclopedia_quiz_prompt(topic: str, article_excerpt: str, grade: int) -> str:
    return f"""Ты — учитель. По материалу об теме «{topic}» составь мини-викторину для закрепления.
Класс: {grade}.

Материал:
{article_excerpt[:2000]}
...
{{
  "questions": [
    {{
      "text": "вопрос",
      "correct_answer": "короткий точный ответ",
      "explanation": "почему так (1 предложение)"
    }}
  ]
}}

Ровно 3 вопроса разной сложности. Ответы — одно слово, число или короткая фраза."""
